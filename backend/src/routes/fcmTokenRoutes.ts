...
router.post('/save', async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        // Dynamically import the appropriate model based on user type
        let UserModel: any;
        switch (userType) {
            case 'Customer':
        ...
        console.log(`✅ FCM token saved for ${userType} user ${userId} (${platform})`);
...
router.delete('/remove', async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        console.log(`✅ FCM token removed for ${userType} user ${userId} (${platform})`);
...
router.post('/test', async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        if (uniqueTokens.length === 0) {
            res.json({
                success: false,
                message: 'No FCM tokens found for this user. Please register a token first.'
            });
            return;
        }
